...
describe('cross-platform desktop packaging', () => {
  it('[version-contract] pins the product release to v5.4.1', async () => {
    ...
    expect(rootPackage.version).toBe('5.4.1');
    expect(desktopPackage.version).toBe('5.4.1');
  });
  ...
  it('[version-contract] keeps every workspace package and runtime version aligned', async () => {
    ...
    for (const packagePath of packagePaths) {
      ...
      expect(packageJson.version, packagePath).toBe('5.4.1');
    }
    ...
    expect(ipcContracts).toContain("APP_VERSION = '5.4.1'");
    expect(shared).toContain("APP_VERSION = '5.4.1'");
  });
