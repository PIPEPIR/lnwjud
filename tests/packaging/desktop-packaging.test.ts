...
describe('cross-platform desktop packaging', () => {
  it('[version-contract] pins the product release to v5.6.1', async () => {
    ...
    expect(rootPackage.version).toBe('5.6.1');
    expect(desktopPackage.version).toBe('5.6.1');
  });
  ...
  it('[version-contract] keeps every workspace package and runtime version aligned', async () => {
    ...
    for (const packagePath of packagePaths) {
      ...
      expect(packageJson.version, packagePath).toBe('5.6.1');
    }
    ...
    expect(ipcContracts).toContain("APP_VERSION = '5.6.1'");
    expect(shared).toContain("APP_VERSION = '5.6.1'");
  });
  ...
  it('publishes complete desktop application metadata', async () => {
    ...
    expect(desktopPackage.description).toBe('Cross-platform local AI-agent runtime and MCP gateway with 276 total tool definitions.');
