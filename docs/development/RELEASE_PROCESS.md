...
9. Every public GitHub Release body uses the same mandatory note structure,
   generated by `scripts/release-notes.mjs`: `## Features`, `## Bug Fixes`,
   `## Other Changes`, then one `**Full Changelog**` link. Empty categories stay
   visible as `- None.` so every version has the same scan-friendly layout.

## Release-note format

Release notes are not free-form. The tag workflow generates them with
`scripts/release-notes.mjs` and publishes that file as the GitHub Release body.
Maintain the headings in this exact order:
...
engasnm111/lnwjud` as a dry run first. Only add `--apply` after reviewing the
reported release count. This operation edits Release descriptions only; it does
not move tags, rebuild artifacts, or alter published binaries.
