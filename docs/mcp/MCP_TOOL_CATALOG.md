...
Current runtime registry: **242 total tool definitions**, with **235 advertised by default**
and **all 242 advertised when the six `codex_*` delegation tools plus `agent_swarm_run` are enabled**. Those seven Codex/Agent Swarm definitions are opt-in; every other current first-party definition is present in normal `tools/list` and reports dependency/setup state truthfully at runtime. The historical 184-tool
