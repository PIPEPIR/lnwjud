...
- **ChatGPT Pro read-only MCP:** when using the Remote MCP gateway, connect the custom app to `/mcp-readonly`. This surface advertises only tools that are internally `READ` and explicitly annotated read-only/non-destructive; write, execute, dangerous, and mixed-action tools remain unavailable. The normal `/mcp` endpoint is unchanged for clients with full MCP support.
