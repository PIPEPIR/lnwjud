...
- **Validation of the source-only hardening:** the durable store suite passed 13 tests with 1 platform-specific skip, the Desktop MCP health/shell scenario passed three consecutive runs, the exact Windows desktop shard passed 53 files / 378 tests, capabilities and Desktop typechecks passed, root lint passed, and `git diff --check` passed without relaxing runtime/test timeouts.
