...
- **Validation of the source-only hardening:** the durable store suite passed 13 tests with 1 platform-specific skip, the Desktop MCP health/shell scenario passed three consecutive runs, the exact Windows desktop shard passed 53 files / 378 tests, the full application suite passed 31 files / 208 tests in about 18 seconds with the fault-injection file under one second, capabilities/application/Desktop typechecks passed, root lint passed, release gate 23/23 passed, and `git diff --check` passed without relaxing runtime/test timeouts.
