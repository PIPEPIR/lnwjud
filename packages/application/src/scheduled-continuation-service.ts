...
export class ScheduledContinuationService {
  ...
  public async claimScheduledContinuation(
    ...
    try {
      ...
      if (request.goalId !== undefined && required(request.goalId, 'goalId', MAX_ID) !== currentContinuation.goalId) {
        throw new GoalStateError('conflict', 'Scheduled continuation goal identity does not match the expected goal');
      }
      const currentGoal = await this.requireOwnedGoal(actor, currentContinuation.goalId);
      if (request.workspaceId !== undefined && required(request.workspaceId, 'workspaceId', MAX_ID) !== currentGoal.workspaceId) {
        throw new GoalStateError('conflict', 'Scheduled continuation workspace identity does not match the expected workspace');
      }
