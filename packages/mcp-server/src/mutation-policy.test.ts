...
const cases: readonly MutationCase[] = [
  ...
  { label: 'ECC configuration persistence', tool: 'ecc_configure', input: { enabled: true, mode: 'selective' }, permission: 'WRITE', kind: 'bounded_write' },
  { label: 'ECC memory create-only save', tool: 'ecc_memory_save', input: { workspaceId: 'w', title: 'note', body: 'body' }, permission: 'WRITE', kind: 'bounded_write' },
  { label: 'ECC AgentShield scan', tool: 'ecc_security_scan', input: { workspaceId: 'w' }, permission: 'EXECUTE', kind: 'execute' },
