...
export const UPGRADE_TOOL_CATALOG: readonly UpgradeToolCatalogEntry[] = [
  ...
  read('windows_environment', 20, 'Inspect Windows environment metadata.', ['windows', 'environment'], { availability: 'optional', requirements: ['Windows host'] }),
  read('service_context', 20, 'Inspect host service-manager metadata.', ['services', 'diagnostics']),
  read('process_context', 20, 'Inspect host process-tree context.', ['process', 'diagnostics']),
  read('port_context', 20, 'Inspect local listening-port context.', ['network', 'diagnostics']),
  read('registry_context', 20, 'Inspect Windows registry context through the Windows capability boundary.', ['windows', 'registry'], { availability: 'optional', requirements: ['Windows host'] }),
  read('event_log_context', 20, 'Inspect host-native event and log context.', ['events', 'diagnostics']),
  read('installed_runtime_context', 20, 'Inspect installed runtimes and package managers.', ['runtime', 'diagnostics']),
  read('path_context', 20, 'Resolve executable and PATH context.', ['path', 'diagnostics']),
  read('startup_context', 20, 'Inspect host startup configuration context.', ['startup', 'diagnostics']),
