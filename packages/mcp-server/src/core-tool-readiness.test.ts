...
describe('core tool readiness', () => {
  it('tracks one representative contract for every core tool in the complete 253-tool inventory', () => {
    const registry = coreRegistry();
    expect(registry.listAll()).toHaveLength(253);
    ...
    expect(coreToolNames(registry)).toHaveLength(106);
  ...
  it('keeps the exhaustive success matrix aligned with all 106 core tools', () => {
    const registry = coreRegistry();
